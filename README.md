...
│   ├── backup-db.sh      # Backup database (chạy tay hoặc qua cron)
│   ├── restore-db.sh     # Khôi phục database từ file backup
│   ├── setup-backup-cron.sh # Cài cron backup tự động
│   ├── logs.sh           # Xem logs
│   └── status.sh         # Kiểm tra hệ thống
├── ecosystem.config.js   # PM2 config
├── install.sh            # Cài đặt lần đầu (one-shot)
├── RUNBOOK.md            # Quy trình backup + khôi phục thảm họa
└── .env.example
```
...
```bash
bash scripts/setup-backup-cron.sh       # cài cron backup tự động (1 lần/ngày mặc định)
bash scripts/backup-db.sh               # backup thủ công ngay
bash scripts/restore-db.sh              # xem danh sách backup có sẵn
bash scripts/restore-db.sh <file>       # khôi phục từ 1 file cụ thể
```

Xem đầy đủ quy trình (bao gồm backup `.env`/`WALLET_ENCRYPTION_KEY`, đồng bộ ra ngoài VPS, và các bước khôi phục thảm họa trên server mới) tại **[`RUNBOOK.md`](./RUNBOOK.md)**.
...
- [ ] Cài cron backup DB tự động: `bash scripts/setup-backup-cron.sh`
- [ ] Cấu hình `BACKUP_REMOTE` để backup đồng bộ ra ngoài VPS (không chỉ nằm 1 chỗ)
- [ ] Backup `.env` + `WALLET_ENCRYPTION_KEY` thủ công, mã hoá, lưu tách biệt khỏi backup DB — xem [`RUNBOOK.md`](./RUNBOOK.md) mục 3
- [ ] Thử restore ít nhất 1 lần trước khi cần dùng thật — xem [`RUNBOOK.md`](./RUNBOOK.md) mục 4
